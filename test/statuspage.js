...
describe('Reading microservices status page', () => {
    let compendium_id = '';
    ...
    describe('GET /status for admin user', () => {
        ...
        it('should response with HTTP 200 OK', (done) => {
            request({ url: global.test_host + '/status', jar: j }, (err, res) => {
                assert.ifError(err);
        ...
        it('should respond with a valid JSON document', (done) => {
            request({ url: global.test_host + '/status', jar: j }, (err, res, body) => {
                assert.ifError(err);
    ...
    describe('GET /status for regular user', () => {
        it('should response with HTTP 401', (done) => {
            request(global.test_host + '/status', (err, res) => {
                assert.ifError(err);
                assert.equal(res.statusCode, 401);
                done();
            });
        ...
        it('should respond with a valid JSON document', (done) => {
            request(global.test_host + '/status', (err, res, body) => {
                assert.ifError(err);
                assert.isObject(JSON.parse(body));
                done();
            });
        ...
        it('should respond with document containing error message', (done) => {
            request(global.test_host + '/status' + compendium_id, (err, res, body) => {
                assert.ifError(err);
                let response = JSON.parse(body);
                assert.property(response, 'error');
                done();
