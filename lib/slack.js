...
exports.start = function (error, done) {
  ...
  bot2r.started(function (payload) {
    debug('Started... payload from rtm.start: %s', util.inspect(payload, { depth: 1, colors: true }));
  });
  ...
  bot2r.hello(function (payload) {
    debug('[hello] connected to server: %o', payload);
    joinChannelAndSayHello(error, done);
  });

  // connect!
  bot2r.listen({ token: config.slack.bot_token }, (err) => {
    if(err) {
      debug('Error starting Slack bot: %o', err);
      error(err);
    }
  });
};
...
joinChannelAndSayHello = function (err, done) {
  ...
  //    debug(err);
  //  else
  //    debug('Response on joining channel %s: %o', config.slack.channel.status, data);
  //});

  slack.chat.postMessage({
    token: config.slack.bot_token,
    channel: config.slack.channel.status,
    text: 'I am now online from *badger* with callback URL `' + config.oauth.default.callbackURL + '` on host `' + os.hostname() + '`.'
  }, (e, data) => {
    if (e) {
      debug(e);
      err(e);
...
exports.newOrcidUser = function (orcid) {
  ...
  slack.chat.postMessage({
    ...
    text: '<!here> A new ORCID user just registered on `' + os.hostname() + '`: ' + user_link,
    attachments: [
      {
        ...
        ts: (new Date).getTime(),
        footer_icon: "https://o2r.info/favicon-16x16.png",
    ...
    if (err) {
      ...
      if (data.ok) {
        ...
        debug('Message send NOT OK. Response for posting new user message: %o', data);
      }
    }
  });
};
...
function sendMessageToSlackResponseURL(responseURL, msg, done) {
  ...
  request(options, (err, response, body) => {
    if (err) {
      debug('Error posting to slack response URL: %o', err);
      done(err);
    } else {
      debug('Posted message to response URL: %O', body);
      done(body);
    }
  });
}
        ...
        if (err) {
          ...
          slack.chat.postMessage({
            ...
            if (e) {
              ...
              debug('Response on posting level change message to %s: %o', config.slack.channel.status, data);
            }
          });
        }
      });
